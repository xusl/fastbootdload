...
CDeviceList::CDeviceList()
{
   ...
   // m_supportDevice.push_back("USB\\VID_1BBB&PID_007A&REV_0228");
    ...
    m_supportDevice.push_back("USB\\VID_067B&PID_2303&REV_0400");
...
//                strTemp.fromWCharArray(deviePath);
				...
				if (type == DEVTYPE_PORT || type == DEVTYPE_MODEM) 
				{
                    ...
                    {
                        string str = szValue;
						StringToWString(str,info->pname);
...
)
{
    ...
    string strHwid = "";
	//BOOL StringToWString(const std::string &str,std::wstring &wstr);
	WStringToString(hwid ,strHwid);
    //strHwid = strHwid.fromStdWString(hwid);
	 transform(strHwid.begin(), strHwid.end(), strHwid.begin(), ::toupper);  
