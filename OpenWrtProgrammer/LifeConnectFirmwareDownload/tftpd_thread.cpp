...
int             Ark;
     ...
     WriteFile (hFile, szidentBuf, 32, (LPDWORD)&Ark, NULL);
     CloseHandle (hFile);

    LOGE("MD5 written into <%s>", szMD5File);
...
struct tftphdr *tp = (struct tftphdr *) pTftp->b.cnx_frame;
   ...
   LOGE("File <%s> : error %d in system call %s %s", tp->th_stuff, GetLastError (), szText, LastErrorText() );
   nak (pTftp, nTftpErr);
return FALSE;
} // TftpSysError
...
char  szExtendedName [2 * _MAX_PATH];
  ...
  tp = (struct tftphdr *)pTftp->b.cnx_frame;
    ...
    {
        LOGW ("Unexpected request %d from peer", opcode);
        LOGW("Returning EBADOP to Peer");
        nak (pTftp, EBADOP);
        return CNX_FAILED;
    }
    ...
    {
        LOGE ("File name too long, return EBADOP to peer");
        nak (pTftp, EBADOP);
        return CNX_FAILED;
    }
    ...
    {
        LOGE ("Error: Tftpd32 does not handle filenames with a percent sign");
        nak (pTftp, EACCESS);
        return  CNX_FAILED;
    }
    LOGD ("FileName is <%s>", tp->th_stuff);
    ...
    LOGD ("%s request for file <%s>. Mode %s",
    ...
    {
        LOGE ("Error EACCESS on file %s. Ext error %s", tp->th_stuff, LastErrorText());
        nak (pTftp, EACCESS);
        return  CNX_FAILED;
    }
    ...
    LOGD ("final name : <%s>", szExtendedName);
    ...
    while ( (sSettings.bNegociate || sSettings.bPXECompatibility)  &&  Ark<TFTP_SEGSIZE)
    {
        ...
        {
            LOGD ("Option <%s>: value <%s>", p, pValue);
            ...
            {unsigned dwDataSize;
                ...
                if (dwDataSize<TFTP_MINSEGSIZE)            // BLKSIZE < 8 --> refus
                     LOGD ("<%s> proposed by client %d, refused by Tftpd32", p, dwDataSize);
                if (dwDataSize>TFTP_MAXSEGSIZE)            // BLKSIZE > 16384 --> 16384
                {
                     LOGD ("<%s> proposed by client %d, reply with %d", p, dwDataSize, TFTP_MAXSEGSIZE);
                     pTftp->s.dwPacketSize = TFTP_MAXSEGSIZE;
                }
                if (dwDataSize>=TFTP_MINSEGSIZE   && dwDataSize<=TFTP_MAXSEGSIZE)
                {
                     LOGD ("<%s> changed to <%s>", p, pValue);
             ...
             if (IS_OPT (p, TFTP_OPT_TSIZE))
             {
                ...
                LOGD ("<%s> changed to <%u>", p, pTftp->st.dwTransferSize);
                bOptionsAccepted = TRUE;
             }  // file size options
			 ...
			 {SOCKADDR_STORAGE   sa;
			  int dummy = sizeof sa;
				 ...
				 {char szServ[NI_MAXSERV];
					 ...
					 LOGW ("using udpport option --> %s", szServ);
                     lstrcpy (pAck, p), pAck += lstrlen (p)+1;
      ...
      int  bKey;
         ...
         for (Ark=0, bKey=0  ; Ark<(int)(pAck-tpack->th_stuff) ; Ark++)
             if (szLog[Ark]==0)
                 szLog[Ark] = (bKey++) & 1 ? ',' : '=';
         LOGD ("OACK: <%s>", szLog);
         LOGD ("Size of OACK string : <%d>", pTftp->c.dwBytes);
...
int nBlock;
    ...
    LOGD ("<%s>: %s %d blk%s, %d bytes in %d s. %d blk%s resent",
                tp->th_stuff,
...
struct tftphdr *tp;
 ...
 // pTftp->c.nCount is the last block sent
    ...
    do
    {
        ...
        for (   ;
                ...
                pTftp->c.nLastToSend ++  )
         {
              ...
              if (pTftp->m.bInit && pTftp->c.nRetries==0)  MD5Update (& pTftp->m.ctx, (unsigned char *)tp->th_data, pTftp->c.dwBytes);
    ...
    if (pTftp->c.nRetries<3  &&  pTftp->c.nCount==4)
        { LOGD ( "dropping 4th packet"), Rc=pTftp->c.dwBytes + TFTP_DATA_HEADERSIZE; }
    else
#endif
        ...
        if (TftpSelect (pTftp))       // something has been received
        {
             ...
             {
                  LOGE("rcvd packet too short");
             }
             else if (ntohs (tp->th_opcode) == TFTP_ERROR)
             {
                  LOGE("Peer returns ERROR <%s> -> aborting transfer", tp->th_msg);
                  return FALSE;
             }
             else if (ntohs (tp->th_opcode) == TFTP_ACK)
             {
                  ...
                  else
                  {
                      ...
                      {
                            LOGE("Ack block %d ignored (received twice)",
                      ...
                      {
                           LOGE("Ack of block #%d received (last block sent #%d !)",
            ...
            {
                  LOGE("ignore unknown opcode %d received", ntohs (tp->th_opcode) );
            } // unknown packet rcvd
        } //something received
        else
        {
              LOGD("timeout while waiting for ack blk #%d", pTftp->c.nCount );
              pTftp->c.nTimeOut ++;
        }   // nothing received
    } // for loop
    ...
    {
         LOGE("MAX RETRIES while waiting for Ack block %d. file <%s>",
    ...
    {
         LOGE("TIMEOUT waiting for Ack block #%d ", pTftp->c.nCount);
         nak (pTftp, EUNDEF);  // return standard
       return FALSE;
    }
    LOGD("Count %d, Last pkt %d ", pTftp->c.nCount, pTftp->c.nLastToSend);
        ...
        if (TftpSelect (pTftp))      // something has been received
     {
          ...
          // it should be a data block
           if (ntohs (tp->th_opcode) != TFTP_DATA)
            {
              LOGE("Peer sent unexpected message %d", ntohs (tp->th_opcode));
                nak (pTftp, EBADOP);
               return FALSE;
          }
          // some client have a bug and sent data block #0 --> accept it anyway after one resend
#ifdef YYYYY
LOGE("block %d, previous acked %d, rcvd bytes %d, retries %d",
ntohs(tp->th_block), pTftp->c.nCount, pTftp->st.dwTotalBytes, pTftp->c.nRetries);
           ...
           if (ntohs(tp->th_block)==0 && pTftp->c.nCount==0)
          {
            ...
            if (pTftp->st.dwTotalBytes==0 && pTftp->c.nRetries==0)
             {
                 LOGE("WARNING: First block sent by client is #0, should be #1, fixed by Tftpd32");
         ...
         if (ntohs (tp->th_block) == (unsigned short) (pTftp->c.nCount+1 ) )
            {
              ...
              if (! Rc)
               ...
               if (pTftp->m.bInit)  MD5Update (& pTftp->m.ctx, (unsigned char *)tp->th_data, pTftp->c.dwBytes);
        ...
        {
               LOGD("timeout while waiting for data blk #%d", pTftp->c.nCount+1 );
             pTftp->c.nTimeOut ++;
        }  // nothing received
   ...
   {
         LOGE("MAX RETRIES while waiting for Data block %d. file <%s>",
   ...
   {
        LOGE("TIMEOUT while waiting for Data block %d, file <%s>",
                 (unsigned short) (pTftp->c.nCount+1), ((struct tftphdr *) pTftp->b.cnx_frame)->th_stuff );
         nak (pTftp, EUNDEF);  // return standard
         return FALSE;
   }
