...
while(true) {
    ...
    LOGD("read %d bytes, '%s'", nread, inbuf);
    ...
    code = telrcv((unsigned char *)inbuf, nread);
    if(code) {
      break;
    }
    }
...
//  curl_socket_t sockfd = conn->sock[num];
  ...
  if(-1 == bytes_written) {
    ...
    if(/* This is how Windows does it */
      ...
      LOGE( "Send failure: %s", Curl_strerror(err));
  ...
  *written = bytes_written;
  ...
  case CURLE_OK:
    LOGE( "Send general failure: %s", Curl_strerror(err));
    /* general send failure */
    //return CURLE_SEND_ERROR;

  default:
    LOGE( "Error: %s", Curl_strerror(err));
  ...
  *buf = '\0';
  ...
  if(err >= 0 && err < sys_nerr)
    ...
    if(!get_winsock_error(err, buf, max) &&
      ...
      _snprintf(buf, max, "Unknown error %d (%#x)", err, err);
  }
