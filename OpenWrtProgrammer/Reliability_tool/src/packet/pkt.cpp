...
===========================================================================*/
#ifndef Q_OS_WIN32
TResult CPacket::Init(const char* device)
{
   if (strlen(device) < 1)
       return EFAILED;

   if (FAILURE(this->m_SerialPort.Open(device)))
   {
           return EFAILED;
   }
...
RcvDIAGPkt_Error:
    //typeqDebug("COM%d: RcvDIAGPkt failure, first %d bytes:", port, MIN(i, BUFFER_HEADER_LEN));
...
#define BUFFER_HEADER_LEN (64)
	...
	** Keep looking until we find one. */
	...
	for (state = HDLC_HUNT_FOR_FLAG; state != HDLC_PACKET_RCVD; /* nil */) 
	{
		...
		if (i < BUFFER_HEADER_LEN) 
		{
			//DEBUG(FILE_LINE, "COM%d: RcvHDLCPkt, byte %d = %d", port, i, chr);
			buffer_header[i] = chr;
			++ i;
		}
		if (FAILURE(result)) 
...
RcvHDLCPkt_Error:
    ...
    //typeqDebug("COM%d: RcvHDLCPkt failure, first %d bytes: ", port, MIN(i, BUFFER_HEADER_LEN));
