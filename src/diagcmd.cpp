...
)
{
	...
	{
                ERR("check_diag_rsp_ok failure, cmd_code = %d, rsp_code = %d!"
                          ,cmd_ptr->buf[0], rsp_ptr->buf[0]);
...
static bool check_nv_status_ok(uint16 status)
{
	...
	{
//		ERR("check_nv_status_ok failure, status = %d", status);
		return false;
	}
...
)
{
	...
	{
		ERR("NV_Read_Item, Invalid params!!!");
		return false;
	}
	...
	*status = 0;
	...
	{
		ERR("NV_Read_Item, cmd is NOT ok!!!");
		return false;
	}
	...
	if (FAILURE(result))
	{
//		ERR("NV_Read_Item SendCmd failure, result = %d", result);
		return false;
	}
	...
	if (FAILURE(result))
	{
//		ERR("NV_Read_Item Receive failure, result = %d", result);
		return false;
	}

	if (rsp_ptr->length != rlen)
	{
//		ERR("NV_Read_Item failure, rsp_ptr->length = %d, rlen = %d!",
//			rsp_ptr->length, rlen);
		return false;
	}
...
)
{
	...
	if ((data == NULL) || (status == NULL))
	{
		ERR("NV_Write_Item, Invalid params!!!");
		return false;
	}
	...
	if (!check_diag_cmd_ok(cmd_ptr))
	{
		ERR("NV_Write_Item, cmd is NOT ok!!!");
		return false;
	}
...
bool CDIAGCmd::RestartDevice()
{
	...
	if (!check_diag_cmd_ok(cmd_ptr))
	{
		ERR("Offline mode, cmd is NOT ok!!!");
		return false;
	}
...
{
	TResult result = EOK;
        efs2_diag_stat_req_type 	req;
        efs2_diag_stat_rsp_type 	rsp;
...
{
	TResult result = EOK;
        efs2_diag_unlink_req_type 	req;
        efs2_diag_unlink_rsp_type 	rsp;
...
{
	TResult result = EOK;
        efs2_diag_read_req_type    req;
        efs2_diag_read_rsp_type    rsp;
	...
	for (;;)
	{
		...
		if (FAILURE(result) || rsp.diag_errno != 0)
		{
//			ERR("COM%d: EfsOpRead failure, result=%d, errno=%d!",
...
{
	TResult result = EOK;
        efs2_diag_write_req_type    req;
        efs2_diag_write_rsp_type    rsp;
	uint32 ilen = 0;
	uint32 rlen = sizeof(rsp);
	...
	for (;;)
	{
		...
		if (FAILURE(result) || rsp.diag_errno != 0)
		{
//			ERR("COM%d: EfsOpWrite failure, result=%d, errno=%d!",
//				m_packetDll->GetComPort(), result, rsp.diag_errno);
			return EFAILED;
		}
		...
		{
//			ERR("COM%d: EfsOpWrite failure, cmd_code=%d, subsys_cmd_code=%d!",
//				m_packetDll->GetComPort(), rsp.hdr.cmd_code, rsp.hdr.subsys_cmd_code);
			return EFAILED;
		}
		if ((int32)len < rsp.bytes_written)
		{
//			ERR("COM%d: EfsOpWrite failure, len (%d) < rsp.bytes_written (%d)!",
...
{
	TResult result = EOK;
        efs2_diag_close_req_type req;
        efs2_diag_close_rsp_type rsp;
	uint32 len  = sizeof(req);
	uint32 rlen = sizeof(rsp);
	...
	if (FAILURE(result) || rsp.diag_errno != 0)
	{
//		ERR("COM%d: EfsOpClose failure, result=%d, errno=%d!",
//				m_packetDll->GetComPort(), result, rsp.diag_errno);
		return EFAILED;
	}
...
TResult CDIAGCmd::EfsOpHello(bool bArmEfs)
{
	...
	if (FAILURE(result))
	{
//		WARN("COM%d: EfsOpHello failure!", m_packetDll->GetComPort());
		return result;
	}
	...
	{
//		ERR("COM%d: EfsOpHello failure, cmd_code=%d, subsys_cmd_code=%d!",
//				m_packetDll->GetComPort(), rsp.hdr.cmd_code, rsp.hdr.subsys_cmd_code);
		return EFAILED;
	}
...
TResult CDIAGCmd::SendRestoreCmd(void)
{
	...
	if (!check_diag_cmd_ok(cmd_ptr))
	{
//		ERR("Send Restore command, cmd is not ok!");
		return EFAILED;
	}
	...
	for (int i=0; i<MAX_RESEND; ++i)
	{
		...
		if (FAILURE(result))
		{
//			LOG("Send Restore command failure, result: %d!", result);
			continue;
		}
...
//			continue;
	...
	if (FAILURE(result))
	{
//		ERR("Send Restore command failure, result: %d!", result);
		return result;
	}
	...
	if (!check_diag_rsp_ok(cmd_ptr, rsp_ptr))
	{
		memset(rsp_ptr->buf, 0x00, sizeof(rsp_ptr->buf));
		rsp_ptr->length = 0;
//		ERR("Send Restore command check_diag_rsp_ok failure!");
		return EFAILED;
	}
...
TResult CDIAGCmd::RequestFirmwareVer_N(char* fdata)
{
	...
	if (fdata == NULL)
	{
//		ERR("COM%d: Request firmware version failure, EINVALIDPARAM!");
	...
	if (!check_diag_cmd_ok(cmd_ptr))
	{
//		ERR("Request firmware version, cmd is not ok!");
		return EFAILED;
	}
	...
	for (int i=0; i<MAX_RESEND; ++i)
	{
		...
		if (FAILURE(result))
		{
//			LOG("Request firmware version failure, result: %d!", result);
			continue;
		}
		...
		if (FAILURE(result))
		{
//			LOG("Receive firmware version failure, result: %d!", result);
			continue;
		}
		break;
	}
	if (FAILURE(result))
	{
//		ERR("RequestFirmwareVer_N failure, result: %d!", result);
		return result;
	}
	...
	if (!check_diag_rsp_ok(cmd_ptr, rsp_ptr))
	{
		memset(rsp_ptr->buf, 0x00, sizeof(rsp_ptr->buf));
		rsp_ptr->length = 0;
//		ERR("RequestFirmwareVer_N check_diag_rsp_ok failure!");
		return EFAILED;
	}
...
//		ERR("Enable diag server, cmd is not ok!");
	...
	for (int i=0; i<MAX_RESEND; ++i)
	{
		...
		if (FAILURE(result))
		{
//			LOG("Enable diag server failure, result: %d!", result);
			continue;
		}
...
TResult CDIAGCmd::GetRootVer_N(uint8& ver)
{
	...
	if (!check_diag_cmd_ok(cmd_ptr))
	{
		ERR("Request root version, cmd is not ok!");
		return EFAILED;
	}
	...
	for (int i=0; i<MAX_RESEND; ++i)
	{
		...
		if (FAILURE(result))
		{
//			LOG("Request root version failure, result: %d!", result);
			continue;
		}
		...
		if (FAILURE(result))
		{
//			LOG("Receive root version failure, result: %d!", result);
			continue;
		}
		break;
	}
	...
	if (FAILURE(result))
	{
//		ERR("GetRootVer_N failure, result: %d!", result);
		return result;
	}
	...
	if (!check_diag_rsp_ok(cmd_ptr, rsp_ptr))
	{
		memset(rsp_ptr->buf, 0x00, sizeof(rsp_ptr->buf));
		rsp_ptr->length = 0;
		ERR("GetRootVer_N check_diag_rsp_ok failure!");
		return EFAILED;
	}
...
//		ERR("Request flash type, cmd is not ok!");
	...
	for (int i=0; i<MAX_RESEND; ++i)
	{
		...
		if (FAILURE(result))
		{
//			LOG("Request flash type failure, result: %d!", result);
			continue;
		}
		...
		if (FAILURE(result))
		{
//			LOG("Receive flash type failure, result: %d!", result);
			continue;
		}
		break;
	}
	if (FAILURE(result))
	{
//		ERR("RequestFlashType_N failure, result: %d!", result);
		return result;
	}
	...
	if (!check_diag_rsp_ok(cmd_ptr, rsp_ptr))
	{
		memset(rsp_ptr->buf, 0x00, sizeof(rsp_ptr->buf));
		rsp_ptr->length = 0;
		ERR("RequestFlashType_N check_diag_rsp_ok failure!");
		return EFAILED;
	}
	type = rsp_ptr->buf[4];
	return EOK;
...
)
{
	...
	if (INdata == NULL)
	{
		ERR("EfsOperate failure, Invalid params!!!");
		return EINVALIDPARAM;
	}
	...
	for (int i=0; i<MAX_RESEND; ++i)
	{
		...
		if (FAILURE(result))
		{
		//	LOG("EfsOperate SendCmd failure, result: %d!", result);
			continue;
		}
		...
		if (FAILURE(result))
		{
		//	LOG("EfsOperate Receive failure, result: %d!", result);
			continue;
		}
		break;
	}
	...
	if (FAILURE(result))
	{
	//	ERR("EfsOperate failure, result: %d!", result);
		return result;
	}
	...
	if (!check_diag_rsp_ok(cmd_ptr, rsp_ptr))
	{
		memset(rsp_ptr->buf, 0x00, sizeof(rsp_ptr->buf));
		rsp_ptr->length = 0;
		ERR("EfsOperate check_diag_rsp_ok failure!");
		return EFAILED;
	}
...
TResult CDIAGCmd::OperDashboardFlag(int operType, int& flag, uint8& retCode)
{
    ...
    if (!check_diag_cmd_ok(cmd_ptr))
    {
        ERR("OperDashboardFlag, cmd is not ok!");
        return EFAILED;
    }
    ...
    for (int i=0; i<MAX_RESEND; ++i)
    {
        ...
        if (FAILURE(result))
        {
            INFO("operation dashboard flag failure, result: %d!", result);
            continue;
        }
        ...
        if (FAILURE(result))
        {
            INFO("Receive OperDashboardFlag failure, result: %d!", result);
            continue;
        }
    ...
    if (FAILURE(result))
    {
        ERR("OperDashboardFlag failure, result: %d!", result);
        return result;
    }
...
{
    TResult result = EOK;
    efs2_diag_opendir_req_type 	req;
    efs2_diag_opendir_rsp_type 	rsp;
...
TResult CDIAGCmd::SendRawRequest(int& retType)
{
    ...
    if (!check_diag_cmd_ok(cmd_ptr))
    {
        ERR("SendRawRequest, cmd is not ok!");
        return EFAILED;
    }
    ...
    for (int i=0; i<MAX_RESEND; ++i)
    {
        ...
        if (FAILURE(result))
        {
            INFO("SendRawRequest, result: %d!", result);
            continue;
        }
        ...
        if (FAILURE(result))
        {
            INFO("Receive SendRawRequest failure, result: %d!", result);
            continue;
        }
    ...
    if (FAILURE(result))
    {
        ERR("SendRawRequest failure, result: %d!", result);
        return result;
    }
    ...
    if (check_diag_rsp_ok(cmd_ptr, rsp_ptr))
    {
        ...
        int iCountL = 0;
        int iCountY = 0;
        for (size_t i=0; i<sizeof(rsp_ptr->buf)/sizeof(rsp_ptr->buf[0]); i++)
        {
            //Find Lighter
            if (rsp_ptr->buf[i] == LBuf[0])
            {
                int k = i;
                for (size_t j=0; j<sizeof(LBuf)/sizeof(LBuf[0]); j++)
                {
            ...
            {
                int k = i;
                for (size_t j=0; j<sizeof(YBuf)/sizeof(YBuf[0]); j++)
                {
...
TResult CDIAGCmd::ReadMacAddress(string & strMacAddress, int iType)
{
    TResult result = EOK;
    diagpkt_subsys_header_type	req;
    uint32 len = sizeof(req);
...
TResult CDIAGCmd::ReadCustModeName(string & strModeName)
{
    TResult result = EOK;
    uint32 rlen = 0;
    DECLARE_CMD_PTR(cmd_ptr);
...
TResult CDIAGCmd::WriteSSID(string strSSID)
{
    TResult result = EOK;
    uint32 rlen = 0;
    DECLARE_CMD_PTR(cmd_ptr);
    ...
    uint8* buf;
    buf = (uint8*)strdup(strSSID.c_str());
...
//add by jie.li 2012-06-27
int32 CDIAGCmd::EfsOpReadDelDir(bool bArmEfs, const char* filename)
{
    TResult result = EOK;
    efs2_diag_readdir_req_type 	req;
    efs2_diag_readdir_rsp_type 	rsp;

    memset(&req, 0, sizeof req);
    memset(&rsp, 0, sizeof rsp);

    req.dirp = EfsOpOpenDir(bArmEfs, filename);
    if (req.dirp < 0)
    {
        return -1;
    }
    ...
    for (int i=1; ; i++)
    {
        ...
        if (rsp.entry_type == 0)
        {
            ...
            {
                result = this->EfsOpCloseDir(bArmEfs, filename, rsp.dirp);
                if (FAILURE(result) || (rsp.diag_errno != 0))
                {
                    return rsp.dirp;
                }
                result = this->EfsOpRMDir(bArmEfs, filename);
                if (FAILURE(result) || (rsp.diag_errno != 0))
                {
                    return rsp.dirp;
                }
            ...
            else
            {
                ...
                result = this->EfsDelFile(bArmEfs, preName);//strName.toAscii().data());
                if (FAILURE(result) || (rsp.diag_errno != 0))
                {
                    return rsp.dirp;
                }
        ...
        else if (rsp.entry_type == 1)
        {
            ...
            else
            {
                ...
                strcat(preName, "/");
                strcat(preName, rsp.entry_name);
                this->EfsOpReadDelDir(bArmEfs, preName);
            }
        }
        else if (rsp.entry_type == 2)
        {
            if (rsp.entry_name[0] == '\0')
            {
                ...
                result = this->EfsDelFile(bArmEfs, filename);
                if (FAILURE(result) || (rsp.diag_errno != 0))
                {
                    return rsp.dirp;
                }
            ...
            else
            {
                ...
                strcat(preName, "/");
                strcat(preName, rsp.entry_name);
                //string	strName(preName);
                result = this->EfsDelFile(bArmEfs, preName);
                if (FAILURE(result) || (rsp.diag_errno != 0))
                {
                    return rsp.dirp;
                }
...
{
    TResult result = EOK;
    efs2_diag_closedir_req_type 	req;
    efs2_diag_closedir_rsp_type 	rsp;
...
{
    TResult result = EOK;
    efs2_diag_rmdir_req_type 	req;
    efs2_diag_rmdir_rsp_type 	rsp;
    uint32 len = sizeof(req) - DIAG_FS_MAX_FILENAME_LEN + strlen(filename) + 1;
    uint32 rlen = sizeof(rsp);
...
{
    TResult result = EOK;
    extend_diag_get_version_req_type  req;
    extend_diag_get_version_rsp_type  rsp;
    ...
    if (fdata == NULL)
    {
        ERR("COM%d: Request Dashboard version failure, EINVALIDPARAM!");
        return EINVALIDPARAM;
    }
    ...
    if (!check_diag_cmd_ok(cmd_ptr))
    {
        ERR("Request Dashboard version, cmd is not ok!");
        return EFAILED;
    }
    ...
    for (int i=0; i<MAX_RESEND; ++i)
    {
        ...
        if (FAILURE(result))
        {
            INFO("Request Dashboard version failure, result: %d!", result);
            continue;
        }
        ...
        if (FAILURE(result))
        {
            INFO("Receive Dashboard version failure, result: %d!", result);
            continue;
        }
        break;
    }
    if (FAILURE(result))
    {
        ERR("RequestDashboardVer failure, result: %d!", result);
        return result;
    }
    ...
    if (!check_diag_rsp_ok(cmd_ptr, rsp_ptr))
    {
        memset(rsp_ptr->buf, 0x00, sizeof(rsp_ptr->buf));
        rsp_ptr->length = 0;
        ERR("RequestDashboardVer check_diag_rsp_ok failure!");
        return EFAILED;
    }
...
TResult CDIAGCmd::RequestQCNVer(char* fdata)
{
    ...
    if (fdata == NULL)
    {
        ERR("COM%d: Request QCN version failure, EINVALIDPARAM!");
        return EINVALIDPARAM;
    }
    ...
    if (!check_diag_cmd_ok(cmd_ptr))
    {
        ERR("Request QCN version, cmd is not ok!");
        return EFAILED;
    }
    ...
    for (int i=0; i<MAX_RESEND; ++i)
    {
            ...
            if (FAILURE(result))
            {
                INFO("Request QCN version failure, result: %d!", result);
                continue;
            }
            ...
            if (FAILURE(result))
            {
                INFO("Receive QCN version failure, result: %d!", result);
                continue;
            }
            break;
    }
    if (FAILURE(result))
    {
        ERR("RequestQCNVer failure, result: %d!", result);
        return result;
    }
    ...
    if (!check_diag_rsp_ok(cmd_ptr, rsp_ptr))
    {
        memset(rsp_ptr->buf, 0x00, sizeof(rsp_ptr->buf));
        rsp_ptr->length = 0;
        ERR("RequestQCNVer check_diag_rsp_ok failure!");
        return EFAILED;
    }
    rsp = *(extend_diag_get_version_rsp_type *)(rsp_ptr->buf);
...
TResult CDIAGCmd::RequestQ6resVer(char* fdata)
{
    ...
    if (fdata == NULL)
    {
        ERR("COM%d: Request Q6Res version failure, EINVALIDPARAM!");
        return EINVALIDPARAM;
    }
    ...
    if (!check_diag_cmd_ok(cmd_ptr))
    {
        ERR("Request Q6Res version, cmd is not ok!");
        return EFAILED;
    }
    ...
    for (int i=0; i<MAX_RESEND; ++i)
    {
        ...
        if (FAILURE(result))
        {
            INFO("Request Q6Res version failure, result: %d!", result);
            continue;
        }
        ...
        if (FAILURE(result))
        {
            INFO("Receive Q6Res version failure, result: %d!", result);
            continue;
        }
        break;
    }
    if (FAILURE(result))
    {
        ERR("RequestQ6ResVer failure, result: %d!", result);
        return result;
    }
    ...
    if (!check_diag_rsp_ok(cmd_ptr, rsp_ptr))
    {
        memset(rsp_ptr->buf, 0x00, sizeof(rsp_ptr->buf));
        rsp_ptr->length = 0;
        ERR("RequestQ6ResVer check_diag_rsp_ok failure!");
        return EFAILED;
    }
    rsp = *(extend_diag_get_version_rsp_type *)(rsp_ptr->buf);
...
{
    TResult result = EOK;
    extend_diag_factory_file_gen_req_type 	req;
...
{
    TResult result = EOK;
    extend_diag_set_func_five_req_type 	req;
