...
int  adb_open(const char*  path, int  options)
{
    ...
    f->fh_handle = CreateFile( (LPCWSTR)path, desiredAccess, shareMode, NULL, OPEN_EXISTING,
                               0, NULL );
...
int  adb_creat(const char*  path, int  mode)
{
    ...
    f->fh_handle = CreateFile((LPCWSTR) path, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
